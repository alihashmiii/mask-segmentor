deleteEdgeSegments[initmask_, selectedpts_] := Module[{pts = selectedpts, mask = initmask, whitepixpos, nearest},
    ...
    nearest = 
     Nearest[whitepixpos, DistanceFunction -> EuclideanDistance];
    pts = 
     pts /. patt : {_, _} /; ! MemberQ[whitepixpos, patt] :> 
       Flatten[nearest[patt], 1]; 
    (* Replace[pts,{x__}/; !MemberQ[
    whitepixpos,{x}]\[RuleDelayed]First@nearest[{x}],{1}] *)
    mask = 
...
interpPoints[selectedpts_, initmask_, interpthresh_] := Module[{pts = selectedpts, ptsToChange, positionReversal, 
    ...
    DeleteDuplicates@
     Flatten[MapAt[Map[Reverse, #] &, interpolatedList, 
       positionReversal], 1]
    
    ] /; Length@selectedpts > 0;

correctSegmentation[image_?ImageQ, correspondingMask_?ImageQ, 
  saveDir_?StringQ] := With[{boxwidth = 10, interp = 0.01, magmask = 5, magimage = 10},
  Module[{img = image, imgmask = correspondingMask, saveLink = saveDir},
   
   zoomMask[x_, y_, immask_] :=
    Module[{row1, row2, col1, col2, 
      imagedim = ImageDimensions[immask]},
     CompoundExpression[
      If[imagedim[[2]] - y - boxwidth <= 0, row1 = 0, 
      ...
      Magnify[ImageTake[
        HighlightImage[
         immask, {Red, AbsolutePointSize[3], 
          Point[Round@{x, y} - 1]}], {row1, row2}, {col1, col2}], 
       magmask]
      ]
     ];
   
   DynamicModule[{list = {}, temp, mouse, mask = imgmask, 
     reset = imgmask, (placeholder = 
       HighlightImage[img, Dynamic@mask]), rule, undo = imgmask},
        ...
        Column@{
          Row@{Magnify[Dynamic@placeholder, magimage],
            Manipulate[
             If[temp =!= None,
              Row@{
                (*Style["pixel val"\[Rule] PixelValue[mask,Round@
                temp],Bold,FontSize\[Rule]18,
       ...
       ButtonBar[{
         "Delete Edge" :> 
          If[Length@list > 0, (undo = mask; 
            mask = deleteEdgeSegments[mask, list]; list = {};)],
         "Add Edge" :> 
          If[Length@list > 0, (undo = mask; 
            mask = ReplacePixelValue[mask, 
               interpPoints[list, mask, interp] -> 1] // Thinning; 
            list = {};)],
         "Undo Operation" :> (mask = undo; list = {};),
         "Reset Mask" :> (mask = reset; list = {}; undo = mask),
         "Show/Hide points" :> If[Length@list > 0,
           
           placeholder = 
            placeholder /. {(rule = 
                HighlightImage[img, Dynamic@mask] -> 
                 HighlightImage[
                  Rasterize@
                   HighlightImage[img, Dynamic@mask], {RGBColor[0.2, 
                    0.6, 0.9], AbsolutePointSize[1], Point@list}]),
               Reverse@rule}],
         "Reset points" :> (list = {};),
